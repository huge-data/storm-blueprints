...
public class DruidBatchStatus {
	...
	public void putInLimbo(Long txId) throws Exception {
		curatorFramework.inTransaction().delete().forPath(CURRENT_PATH + "/" + txId).and().create()
				.forPath(LIMBO_PATH + "/" + txId).and().commit();
	}
	...
	public void complete(List<Long> txIds) throws Exception {
		...
		while (iterator.hasNext()) {
			...
			transaction = transaction.delete().forPath(LIMBO_PATH + "/" + txId).and().create()
					.forPath(COMPLETED_PATH + "/" + txId).and();
		}
