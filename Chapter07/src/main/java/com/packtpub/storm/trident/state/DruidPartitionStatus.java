...
public class DruidPartitionStatus {
	...
	public void putInLimbo(String partitionId) throws Exception {
		curatorFramework.inTransaction().delete().forPath(CURRENT_PATH + "/" + partitionId).and().create()
				.forPath(LIMBO_PATH + "/" + partitionId).and().commit();
	}
	...
	public void complete(List<String> partitionIds) throws Exception {
		...
		while (iterator.hasNext()) {
			...
			transaction = transaction.delete().forPath(LIMBO_PATH + "/" + partitionId).and().create()
					.forPath(COMPLETED_PATH + "/" + partitionId).and();
		}
