...
public class DruidState implements State {
	...
	private final Vector<FixMessageDto> messages = new Vector<>();
	...
	public void commit(Long batchId) {
		...
		try {
			if (StormFirehose.STATUS.isCompleted(partitionId)) {
				...
				LOG.warn("Encountered limbo partition [" + partitionIndex + "] of batch [" + batchId
						+ "] : NOTIFY THE AUTHORITIES!");
				return;
			} else if (StormFirehose.STATUS.isInProgress(partitionId)) {
				LOG.warn("Encountered in-progress partition [\" + partitionIndex + \"] of batch [" + batchId
						+ "] : NOTIFY THE AUTHORITIES!");
				return;
			}
